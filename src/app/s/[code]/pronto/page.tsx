...
import { PAY_COPY, payLang } from "@/app/pagar/copy";
...
export default async function ProntoPage({ params, searchParams }: PageProps) {
  ...
  const business = signup && !["descartada", "cancelada"].includes(signup.status) ? signup.business_name : null;
  const ask = business
    ? `Hola, soy ${business} (código ${signup?.referral_code}). ¿Cómo va mi web?`
    : "Hola, quiero saber cómo va mi web.";
  return (
    <PayShell t={t} lang={lang} toggleHref={`/s/${encodeURIComponent(code)}/pronto?lang=${lang === "es" ? "en" : "es"}`}>
