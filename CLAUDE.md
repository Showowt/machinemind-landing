# Build Rules
- Write COMPLETE files, never partial edits
- Run npm run build after changes. Fix errors until clean.
- Deploy: git add -A && git commit -m "update" && git push origin main
- Do NOT ask questions. Make the best decision and execute.

# Design Rules  
- Background: #06060a ALWAYS. No white anywhere.
- Fonts: Clash Display (headings), Satoshi (body), Instrument Serif (italic accents)
- Colors: gold #c9a96e, cyan #00e5ff, dim rgba(240,240,243,0.3)
- Cards: glass bg, glass border, gold top-border on hover. NO rounded corners. NO shadows.
- Buttons: 1px gold border, fill slides up on hover, text inverts.
- Animations: GSAP ScrollTrigger, opacity 0→1, y 40→0, stagger 0.1
- Overlays: film grain 3% opacity, vignette, scroll progress bar
- BANNED: white bg, Inter/Roboto, purple gradients, rounded shadow cards, emoji

# What MachineMind Is
AI automation consultancy. Sofia AI (WhatsApp), Cinema Engine (websites), Viceroy (private capital).
